!(function (M, z) {
  ...
  function O(M) {
    var z = 0,
      ...
      q = 0,
      o = 1;
    for (45 === M.charCodeAt(0) && (o = -(z = 1)); z < p.length; z++)
      q = 60 * q + c(p.charCodeAt(z));
    for (z = 0; z < O.length; z++) (A /= 60), (q += c(O.charCodeAt(z)) * A);
    return q * o;
  }
  function R(M) {
  ...
  function N(M, z) {
    for (var b, p; (p = 6e4 * (((z.at - M.at) / 12e4) | 0)); )
      (b = new L(new Date(M.at + p))).offset === M.offset ? (M = b) : (z = b);
    return M;
  }
  function a(M, z) {
  ...
  function u() {
    ...
    var b,
      ...
      A = (function () {
        ...
        for (b = 1; b < 48; b++)
          (z = new L(new Date(p, b, 1))).offset !== O.offset &&
            ((M = N(O, z)), A.push(M), A.push(new L(new Date(M.at + 6e4)))),
            (O = z);
        for (b = 0; b < 4; b++)
      ...
      o = (function (M) {
        ...
        for (z in A) A.hasOwnProperty(z) && q.push(W[z]);
        return q;
      })(A),
      c = [];
    for (p = 0; p < o.length; p++) {
      for (b = new f(e(o[p]), q), O = 0; O < q; O++) b.scoreOffsetAt(A[O]);
      c.push(b);
    }
    return c.sort(a), 0 < c.length ? c[0].zone.name : void 0;
  }
  function i(M) {
  ...
  function e(M, z) {
    ...
    return p instanceof n
      ...
      : 'string' == typeof p
        ? ((p = new n(p)), (q[M] = p))
        : o[M] && z !== e && (b = e(o[M], e))
          ? ((p = q[M] = new n())._set(b), (p.name = W[M]), p)
          : null;
  }
  function r(M) {
    var z, b, p, O;
  ...
  function F(M) {
    H(M.zones), r(M.links), (t.dataVersion = M.version);
  }
  function T(M) {
    var z = 'X' === M._f || 'x' === M._f;
  ...
  function t(M) {
    ...
    return (
      p && !A.isMoment(M) && T(O) && O.add(p.parse(O), 'minutes'), O.tz(b), O
    );
  }
  (b < 2 || (2 === b && p < 6)) &&
    s(
      ...
      parse: function (M) {
        ...
        for (O = 0; O < c; O++)
          if (
            ...
            (b = q[O + 1]),
            (p = q[O ? O - 1 : O]),
            z < b && t.moveAmbiguousForward
              ? (z = b)
              : p < z && t.moveInvalidForward && (z = p),
            A < o[O] - 6e4 * z)
          )
            return q[O];
        return q[c];
    ...
    (t.names = function () {
      var M,
        z = [];
      for (M in W)
        W.hasOwnProperty(M) && (q[M] || q[o[M]]) && W[M] && z.push(W[M]);
      return z.sort();
    }),
    (t.Zone = n),
    (t.unpack = B),
  ...
  (A.tz = t),
    ...
    (A.updateOffset = function (M, z) {
      ...
      void 0 === M._z &&
        ...
        M._z &&
          ((b = M._z.utcOffset(M)),
          Math.abs(b) < 16 && (b /= 60),
          void 0 !== M.utcOffset ? M.utcOffset(-b, z) : M.zone(b, z));
    }),
    (S.tz = function (M, z) {
      if (M) {
        if ('string' != typeof M)
    ...
    (S.zoneName = C(S.zoneName)),
    (S.zoneAbbr = C(S.zoneAbbr)),
    (S.utc =
      ((E = S.utc),
      function () {

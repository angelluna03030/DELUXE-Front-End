...
export const ModalCategoria = ({ selectedCategoria, onCategoriasChange }) => {
  ...
  return (
    <>
      <Button onPress={onOpen} categoria="primary" color='primary'>Categorías</Button>
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} isDismissable={false} isKeyboardDismissDisabled={true} size="4xl">
        <ModalContent>
          ...
          <ModalBody>
            <CheckboxGroup value={categoriasSelect} onChange={handleCheckboxChange}>
              <div className="grid sm:grid-cols-6 sm:gap-10 grid-cols-3 gap-2">
                {categorias.map((categoria) => (
                  <label key={categoria._id} className="flex flex-col items-center border-2 border-gray-300 px-3 pt-3 rounded-2xl">
                    <div className="flex">
                      <img
          ...
          <ModalFooter className="mr-48 sm:mr-0 sm:mt-5">
            ...
            <Button categoria="primary" onPress={() => {

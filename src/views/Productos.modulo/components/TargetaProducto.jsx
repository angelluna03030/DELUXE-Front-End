...
import { Chip, Pagination, Card, CardHeader, CardBody, CardFooter, Divider } from "@nextui-org/react";
						...
						<Card key={item.id_producto}>
							<CardHeader className="flex gap-3">
								<div className="flex flex-col">
									<p className="text-small text-default-500">Id de producto: {item.id_producto}</p>
								</div>
							</CardHeader>
							<Divider />
							<CardBody>
									...
									<Grid item xs={4}>
										{
											item.estado_producto === 2 ?
												...
												<Chip
													className='capitalize'
													color={
														statusColorMap[item.estado_producto === 1 ? 'activo' : 'inactivo']
													}
													size='sm'
													variant='flat'
													onClick={() => handleChipClick(item.id_producto)}
